...
# Import notification cache if available
try:
...
# Import Divine Paper Trading System
try:
    from src.paper_trading import DivinePaperTrader
...
class TradingBot:
    ...
    def __init__(self, test_mode: bool = False):
        ...
        # Strategy weights for signal allocation
        self.supertrend_adx_weight = float(os.getenv('SUPERTREND_ADX_WEIGHT', '60'))
        self.inside_bar_weight = float(os.getenv('INSIDE_BAR_WEIGHT', '40'))
        
        # Load confidence threshold from environment variable (user spec: 90%+)
        self.confidence_threshold = float(os.getenv('CONFIDENCE_THRESHOLD', '90'))
        logger.info(f"Signal confidence threshold set to: {self.confidence_threshold}%")
        
        # Maximum number of signals and trades per day (user specs: 100 signals, 20 trades)
        self.max_signals_per_day = int(os.getenv('MAX_SIGNALS_PER_DAY', '100'))
        ...
        # Initialize Divine Paper Trading System
        if PAPER_TRADING_AVAILABLE:
            self.paper_trader = DivinePaperTrader(initial_balance=10.17)
            logger.info("🌙 Divine Paper Trading System initialized")
        else:
            self.paper_trader = None
            logger.warning("Paper trading system not available")
        ...
        if TELEGRAM_AVAILABLE:
            ...
            if self.telegram.is_configured:
                self.telegram_token = self.telegram.token
    ...
    def scan_markets(self):
        ...
        try:
            ...
            # Limit signals to prevent system overload - take only top 10 highest confidence
            high_confidence_signals = sorted(high_confidence_signals, key=lambda x: x['confidence'], reverse=True)[:10]
            logger.info(f"Limited to top {len(high_confidence_signals)} highest confidence signals to prevent overload")
    ...
    def send_signal(self, signal: Dict) -> bool:
        ...
        Returns:
            bool: Success status
        """
        # Use TelegramNotifier attributes if available
        telegram_token = self.telegram_token or (self.telegram.token if self.telegram and hasattr(self.telegram, 'token') else None)
        telegram_chat_id = self.telegram_chat_id or (self.telegram.chat_id if self.telegram and hasattr(self.telegram, 'chat_id') else None)
        ...
        if signal_key in self.recent_signal_keys:
            ...
            # If the same signal was sent in the last 1 hour, skip it (reduced from 4 hours)
        ...
        try:
            ...
            # Execute divine paper trade with complete transparency
            if self.paper_trader:
                logger.info(f" Executing divine paper trade for {signal['symbol']}")
                
                # Execute paper trade
                paper_result = self.paper_trader.execute_paper_trade(signal)
                
                if paper_result.get('success'):
                    # Add execution details to message
                    message += f"\n\n **DIVINE PAPER TRADE EXECUTED**"
                    message += f"\n **Position Value:** ${paper_result['position_value']:.2f}"
                    message += f"\n **Quantity:** {paper_result['quantity']:.4f}"
                    message += f"\n **Trade ID:** {paper_result['trade_id']}"
                    message += f"\n **The immortal consciousness acts with divine precision!**"
                    
                    logger.info(f" Divine paper trade executed: {paper_result['trade_id']}")
                else:
                    message += f"\n\n **PAPER TRADE ERROR:** {paper_result.get('error', 'Unknown error')}"
                    logger.error(f"Paper trade error: {paper_result.get('error')}")
            
            # Also attempt real trade if balance allows (for future when balance is increased)
            if not self.test_mode:
                ...
                if api.is_configured:
                    ...
                    if symbol in self.active_trades:
                        logger.info(f"Skipping real trade for {symbol} as there's already an active trade")
                    else:
                        # Attempt real trade (will fail gracefully if insufficient balance)
                        try:
                            real_result = api.execute_signal(signal)
                            if real_result.get('success'):
                                message += f"\n\n **REAL TRADE ALSO EXECUTED!**"
                                logger.info(f" Real trade executed: {real_result}")
                            elif 'balance' in real_result.get('error', '').lower():
                                message += f"\n\n **Real trade skipped:** Insufficient balance (${api.get_account_info().get('available_balance', 0):.2f})"
                                logger.info("Real trade skipped due to insufficient balance")
                        except Exception as e:
                            logger.warning(f"Real trade attempt failed: {e}")
                    ...
                    try:
                        ...
                        if 'error' in account_info:
                            ...
                            if notification_cache and notification_cache.should_send('error', key=error_key):
                        ...
                        position_value = available_balance * 0.35  # Use 35% of balance per trade (user spec)
                        
                        # Ensure minimum position value of $5 to meet Bitget requirements
                        MIN_POSITION_VALUE = 5.0  # $5 USD minimum (lowered to match current balance)
                        if position_value < MIN_POSITION_VALUE:
                            if available_balance >= MIN_POSITION_VALUE:
                                ...
                                if notification_cache and notification_cache.should_send('insufficient_balance', key=symbol_key):
                        ...
                        if 'error' in market_data:
                            ...
                            current_price = float(market_data.get('last_price', price))
                            
                        # Calculate quantity based on position value and current price
                        quantity = position_value / current_price
                        
                        # Place the order with OCO - Enhanced with better error handling
                        ...
                        # Enhanced result processing with better TP/SL verification
                        if 'main_order' in result and 'error' not in result['main_order']:
                            ...
                            # Add comprehensive order details to message
                            message += f"\n\n📊 *Order Details:*"
                            main_order_id = result['main_order'].get('orderId', 'N/A')
                            message += f"\n• Main Order ID: {main_order_id}"
                            message += f"\n• Symbol: {symbol}"
                            message += f"\n• Direction: {direction}"
                            message += f"\n• Quantity: {quantity:.4f}"
                            
                            # Check TP/SL status with detailed reporting
                            tp_success = result.get('take_profit_order') and 'error' not in result.get('take_profit_order', {})
                            sl_success = result.get('stop_loss_order') and 'error' not in result.get('stop_loss_order', {})
                            
                            if tp_success:
                                tp_order_id = result['take_profit_order'].get('orderId', 'N/A')
                                message += f"\n• Take-Profit: ✅ (ID: {tp_order_id})"
                            else:
                                tp_error = result.get('take_profit_order', {}).get('error', 'Failed to set')
                                message += f"\n• Take-Profit: ❌ ({tp_error})"
                                logger.error(f"TP/SL ISSUE: Take-profit failed for {symbol}: {tp_error}")
                                
                            if sl_success:
                                sl_order_id = result['stop_loss_order'].get('orderId', 'N/A')
                                message += f"\n• Stop-Loss: ✅ (ID: {sl_order_id})"
                            else:
                                sl_error = result.get('stop_loss_order', {}).get('error', 'Failed to set')
                                message += f"\n• Stop-Loss: ❌ ({sl_error})"
                                logger.error(f"TP/SL ISSUE: Stop-loss failed for {symbol}: {sl_error}")
                            
                            # Alert if TP/SL failed but main order succeeded
                            if not tp_success or not sl_success:
                                message += f"\n\n⚠️ *WARNING: Trade executed but TP/SL incomplete!*"
                                message += f"\nManual monitoring required for {symbol}"
